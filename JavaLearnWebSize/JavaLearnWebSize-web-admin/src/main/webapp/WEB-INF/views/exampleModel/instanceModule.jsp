...
<div class="row">
    <div class="column left" style="border:1px solid #DBDAD7 ;">
        <div style="text-align:center; background-color:#F4F4F4;">
            <label class="fontSize">java基础知识</label>
        </div>
        ...
        <c:forEach var="title" items="${sessionScope.Java面向对象}" varStatus="status">
            <div class="youmulu">
                <a href="/queryTutorial?specialSubject=Java面向对象&exampleType=${title}">${title}</a>
            </div>
        </c:forEach>
        ...
        <c:forEach var="title" items="${sessionScope.Java高级教程}" varStatus="status">
            <div class="youmulu">
                <a href="/queryTutorial?specialSubject=Java高级教程&exampleType=${title}">${title}</a>
            </div>
        </c:forEach>
    </div>
    ...
    <div class="column middle">
        <div class="teacherButton" ${sessionScope.user.getUserCharactor() == 0?"style='display: none'":" "}>
            <div class="btn-group">
                ...
                <a href="/modifyExample?specialSubject=${requestScope.specialSubject}&exampleType=${requestScope.exampleType}&dtNum=${requestScope.detailsTutorial.getDtNum()}" class="btn btn-default">修改实例</a>
            </div>
        </div>
        ...
        <div style="font-size: x-large;" class="fontSize">${requestScope.detailsTutorial.getTitle()}</div>
        <br/>
        <div style="background-color: #E5EECC;">
            <div style="font-weight: 700;" class="fontSize">实例</div>
            <pre>${requestScope.detailsTutorial.getContext()}</pre>

        </div>
        <br/>
        <div style="background-color: #E5EECC">
            <div style="font-weight: 700;" class="fontSize">运行结果</div>
            <pre>${requestScope.detailsTutorial.getRunResult()}</pre>
        </div>
        <div style="background-color: #E5EECC">
            <div style="font-weight: 700;" class="fontSize">实例讲解</div>
            <pre>${requestScope.detailsTutorial.getExplainContext()}</pre>
        </div>


        <div>
            <a href="<c:url value='/downloadFile'/>?id=${requestScope.detailsTutorial.getFilePath()}">下载附件</a>
        </div>
